...
',
  ...
  js = "// No example available yet",
  python = "# No example available yet",
  r = '# No example available yet',
...
langs <- langs %>%
  mutate(
    ...
    test_path = paste0(temp_dir, "/", id, "/test.", extension),
  )
pwalk(langs, function(id, script_type, example_config, example_script, config_path, script_path, test_path, ...) {
  # create dir
  ...
  # modify helper and write to file
  readr::write_lines(tests[[id]], test_path)
})
```
