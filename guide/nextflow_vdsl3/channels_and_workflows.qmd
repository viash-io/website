...
Contents of `guide/nextflow_vdsl3/workflows/200-first_nextflow_pipeline/main.nf`:

```{embed, lang="groovy"}
guide/nextflow_vdsl3/workflows/200-first_nextflow_pipeline/main.nf
```
...
```{embed, lang="groovy"}
guide/nextflow_vdsl3/workflows/201-first_nextflow_pipeline_revisited/main.nf
```
...
```{swirly echo=FALSE, label="fig-dataflow", fig.cap="The data flow of `guide/nextflow_vdsl3/workflows/200-first_nextflow_pipeline/main.nf`."}
-a-b-c-d----------|
a := " a "
b := "   b"
c := "  c"
...
[^well]: Actually, there is at least one exception we've bumped into with consequences for how VDSL3's API is defined: function overloading is not available in Nextflow code.
