...
When running a Viash component with `viash run`, Viash will wrap your script into a Bash executable. By passing arguments to the component, Viash will replace add your parameter values to your script by replacing the Viash placeholder code block. This is what a Viash placeholder code block typically looks like:
...
If no such code block exists yet, it is inserted at the top of the file.

At runtime, the Viash placeholder code block is replaced by two maps (or dictionaries): `par` and `meta`. The `par` map contains the parameter values specified by the user, and `meta` contains additional information on the current runtime environment.
