...
|-|:----|-:
| `--config_mod`, `-c` | Modify a viash config at runtime using [dynamic config modding](/reference/config/config_mods/#). | `List[String]` |
...
| Argument | Description | Type |
|-|:----|-:
| `--apply_platform`, `-a` | Fills in the {platform} field by applying each platform to the config separately. Note that this results in the provided command being applied once for every platform that matches the --platform regex. | `Boolean` |
