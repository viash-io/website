...
To get insight into how `par` and `meta` are defined, you can run [`viash config inject`](/reference/cli/config.qmd) to replace the current parameter placeholder with an auto-generated parameter placeholder.
...
<!-- TODO: Write a bit more indepth description -->
...
Path to where the resources are stored. See the [previous section](add-resources.qmd) on how to define resources.

<!-- TODO: Write a bit more indepth description -->

### `temp_dir` (string)

A temporary directory in which your script is allowed to create new temporary files / directories. By default, this will be set to the `VIASH_TEMP` environment variable. When the `VIASH_TEMP` variable is undefined, `/tmp` is used instead.
