...
6. Run the `remove_comments` module with [auto publishing](/documentation/reference/config/platforms/nextflowVdsl3Platform.html#auto) enabled using the `auto` directive, this makes sure that the output of this module is written to a directory based on its `id`
