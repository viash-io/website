...
This will add Nextflow as a build target, with [VDSL3](/documentation/reference/config/platforms/nextflowVdsl3Platform.html) set as the pipeline scripting variant, which adds new features and quality of life changes.
...
If you don't add `docker` as a build target or want to override the image, you can use the [`directives`](/documentation/reference/config/platforms/nextflowVdsl3Platform.html#directives) dictionary to specify a base image to use:
...
In contrast to native and Docker based components, a Nextflow based component cannot be ran by using the [`viash run`](/documentation/reference/viash/run.html) command. Components targeting Nextflow require you to use [`viash build`](/documentation/reference/viash/build.html) first to generate a Nextflow module and run that module using `nextflow run`. See the [Nextflow Build Target](/documentation/guide/building-block/nextflow/building-running.html) page for more information on how to generate a Nextflow module and run it as a standalone pipeline.