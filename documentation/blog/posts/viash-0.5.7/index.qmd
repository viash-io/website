...
A handy new command has been added: `viash config inject`. This can be used to inject a Viash header into a script based on the arguments of the [config file](/api/config/config/).
...
You can now include [ADD](https://docs.docker.com/engine/api/builder/#add) and [COPY](https://docs.docker.com/engine/api/builder/#copy) instructions in the [config file](/api/config/config/):
